...
class Day6TrashCompactor {
    @Test
    fun executePartOne() {
        val problems = ProblemListParser().parse(problems)
        val output = problems.sumOf { it.calculate() }
        println(output)
    }

    @Test
    fun executePartTwo() {
        val problems = ProblemListParser().parseCephalopod(problems)
        val output = problems.sumOf { it.calculate() }
        println(output)
    }

    companion object {
        private val exampleProblems = """
            123 328  51 64 
             45 64  387 23 
              6 98  215 314
            *   +   *   +  
...
data class Problem(
    ...
    fun calculate(): ULong {
        return numbers.drop(1).fold(numbers.first()) { a, b ->
            when (operation) {
                Operation.ADD -> a + b
                Operation.MULIPLY -> a * b
            }
        }
    }
    ...
    companion object {
        fun fromStrings(strings: List<String>): Problem {
            val numbers = strings.dropLast(1).map {
                it.toULong()
            }

            val operation = strings.last().let {
                when (it) {
                    "+" -> Operation.ADD
                    "*" -> Operation.MULIPLY
...
class ProblemListParser {
    fun parse(string: String): List<Problem> {
        val lines = string.lines()
        val rows = lines.map { it.split(Regex("\\s+")).filter { it.isNotEmpty() } }

        val problems = (0..rows.first().lastIndex).map { index ->
            rows.map { it[index] }
        }

        println(problems)

        val mappedProblems = problems.map { problem ->
            Problem.fromStrings(problem)
        }

        // println(mappedProblems)

        return mappedProblems
    }

    fun parseCephalopod(string: String): List<Problem> {
        val columnLengthsRegex = Regex("([*|+]\\s+)(?=\\s?[+|*])?")
        val columnLengths = columnLengthsRegex.findAll(string.lines().last())
            .let {
        ...
        val rows = lines.map { line ->
            val iterator = columnLengths.iterator()
            var remainingLine = line

            buildList {
                for (i in iterator) {
                    add(remainingLine.take(i))
                    remainingLine = remainingLine.drop(i + 1)
                }
            }
        }
        ...
        val cephalopodProblems = problems.map { problem ->
            val numbers = problem.dropLast(1)
            (0..numbers.first().lastIndex).map { index ->
                buildString {
                    println(problem)
                    numbers.forEach {
                        append(it[index])
            ...
            } + listOf(problem.last().trim())
        }

        println("Problems: $cephalopodProblems")

        val mappedProblems = cephalopodProblems.map { problem ->
            Problem.fromStrings(problem)
        }
